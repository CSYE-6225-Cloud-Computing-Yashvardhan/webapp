...
describe("Integration Test", () => {
    ...
    it('Create user account and validate using GET request', async () => {
        ...
        const createUserApiResponse = await request(app).post("/v2/user").send(userData);
        ...
        const getUserApiResponse = await request(app).get(`/v2/user/self`).set('Authorization', `Basic ${Buffer.from(`${userData.email}:${userData.password}`).toString('base64')}`);
    ...
    it('Testing CREATE request without payload', async () => {
        const getUserApiResponse = await request(app).post(`/v2/user/`);
    ...
    it('Update user account and validate using GET request', async () => {
        ...
        const updateUserApiResponse = await request(app).put("/v2/user/self")
    ...
    it('Testing not allowed http methods', async () => {
        const getUserApiResponse = await request(app).options(`/v2/user/self`).set('Authorization', `Basic ${Buffer.from(`${userData.email}:${userData.password}`).toString('base64')}`);
        expect(getUserApiResponse.statusCode).toBe(405);
    });

    it('Testing with invalid user', async () => {
        const getUserApiResponse = await request(app).get(`/v2/user/self`).set('Authorization', `Basic ${Buffer.from("invalid@email.com:12345").toString('base64')}`);
        expect(getUserApiResponse.statusCode).toBe(401);
    });

    it('Testing GET request with payload', async () => {
        const getUserApiResponse = await request(app).get(`/v2/user/self`).send(userData).set('Authorization', `Basic ${Buffer.from("invalid@email.com:12345").toString('base64')}`);
        expect(getUserApiResponse.statusCode).toBe(400);
    });
